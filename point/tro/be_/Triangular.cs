...
namespace nilnul.geometry.planar.point.trio.be_
{
		...
		,trio.BeDblI
	{
		...
		///		If the three points are quotient, keep this within quotient.
		/// </remarks>
		public bool be(Trio obj)
		{
			...
			/// let's see if the two vectors are collinear
			/// 
			/// get their cross product; see if the distance it's zero.
			/// Note: in getting the cross product, the coefficient of i & j are both zero.
			/// 
			/// if we stack two vectors to get a matrix and compute the determinant of the matrix, the determinant is zero if the two vectors are collinear.
		...
		public bool be(TrioD obj) {
			...
			return !nilnul.num.real.be_.AboutNil4Dbl.Injected.be(
				...
				planar.cloze_.trigon.parlgrm._SurroundedX._Surround(
					obj.a, (obj.b)
					,
					obj.c
				)
