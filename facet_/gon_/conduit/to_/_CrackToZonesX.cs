...
namespace nilnul.geometry.planar.facet_.gon_.conduit.to_
{
	...
	/// <summary>
	/// </summary>
	///That is complex for us now. Let's defer it:
	...
	/// for cutting into at most two parts, we need to conduit the holes, zigzagly some times in order to remain  inside the hull. 
	/// </remarks>
	///
	/// 
	/// alias:
	...
	/// <see cref="gon.to_._SplitToAdjacentZonesX"/>
	/// 
	/// todo: to ken
	/// 
	//[InWorking]
...
planar.ray_.horizon_.decrease.vsGrad._NearestJoint.NearestJoint(
			...
			(List<Point4dblI> first, List<Point4dblI> second) cutHole()
			{
				...
				/// some hole might be misswapped. <see cref="unitest uxp"/>
				///

				//var hull9holes = coil_.gon_.conduit._HullAndHolesX._HullWithHoles_0conduit(first);

				//var misplaced1st= hull9holes.holes.Where(
				//	h => !cycle_.gon.co_.unjoint_.pro9con.be_.concord_._Seige4dblX._Be_0pro_1conUnjoint(
				//		hull9holes.hull.Select(e=>e.basis),h.Select(e=>e.basis)
				//	)
				//);

				//var misplaced2nd= coil_.gon_.conduit._HullAndHolesX._HullWithHoles_0conduit(second).holes.Where(
				//	h => !cycle_.gon.co_.unjoint_.pro9con.be_.concord_._Seige4dblX._Be_0pro_1conUnjoint(
				//		hull9holes.hull.Select(e=>e.basis),h.Select(e=>e.basis)
				//	)
				//);
		...
		static public IEnumerable<IEnumerable<Point4dblI>> _ToAdjacentZones_0conduit(
			...
			)
			{
				...
				if (childPoints.Any())
				{
					...
					var splited3 = _CutHole(
						...
						innerMin
						,hullPoints
					);					/// some holes might be mis-swapped;
