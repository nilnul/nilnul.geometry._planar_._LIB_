//using nilnul.geometry.planar.zone_.polygon_.points;
...
namespace nilnul.geometry.planar.zone_.gon_.convex.vsLead
{
	...
	/// <summary>
	/// use a lead to slice the polygon. on the left is one; on the right is another one. 
	/// </summary>
	static public class _SectX
	{
		/// <summary>
		...
		/// </remarks>
		/// 
		static public IEnumerable<PointI1> Eval(point.str_.SeqI _points_compressed, planar.RayI sectingLead)
		{
			...
			var polygonHalf0_latter = new List<PointI1>(); // when looping thru grads, the grads looped before "lead" intersect any grad (in the middle or at the end)

			var polygonHalf1 = new List<PointI1>();    //when looping thru grads, after grads that are looped before any crossing, the grads crossed by "lead"

			var polygonHalf0_former = new List<PointI1>(); // previous to sectedPart0 (if sectedPart0 is at the leeway, then this part is also at the leeway)
			...
			for (int i = 0; i < arrows.Count(); i++) //loop thru all points of  grads. Note this method is around the points, not the ray; we traverse points, when met with ray, deal with it, and then continue to traverse
			{
				...
				var intersectionResult = _LayoutX.Result(sectingLead, currentArrow);

				if (
					intersectionResult is lead.vsArrow._layout.ret_.fork_.MiddleOrEnd4RealI intersectionResultAsIntersection //intersection point is valid (in arrow)
				)
				{
					if ((intersectionResult is lead.vsArrow._layout.ret_.fork_.FroLeewayRealI))  
					{
					...
					if (!(intersectionResult is lead.vsArrow._layout.ret_.fork_.AtStop4RealI) )
					{
					
						polygonHalves[currentSectedIndex].Add(currentArrow.points.ee.end);
					}
				}
				else if (intersectionResult is lead.vsArrow._layout.ret_.align_.Incident resultAsIncident)
				{
					...
					else
					{
						...
						polygonHalves[currentSectedIndex].Add(currentArrow.points.ee.end);  //supposing we let the ray cross the polygon in an infinitely close line parallel to the arrow
					}
					currentSectedIndex++;
					polygonHalves[currentSectedIndex].Add(currentArrow.points.ee.end);
