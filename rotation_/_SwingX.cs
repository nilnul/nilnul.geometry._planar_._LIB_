using System;

namespace nilnul.geometry.planar.rotation_
{
	/// <summary>
	/// 
	/// </summary>
	static public class _SwingX {
		static public double _Swing_0rotation(double rotation) {
			return swing.of_._rotation_.by_._ByShiftX._Swing_0rotation(
				DirectionDbl._OfRotation(rotation)
			);


		}

		static public double _Swing_0direction(double rotation) {
			return rotation >= Math.PI ? rotation - num.real_.eg_._Tau4dblX.FULL : rotation;
		}

	}
}
