...
namespace nilnul.geometry.planar.angle_
{
	...
	/// </summary>
	public class ByPoints : geometry.planar.point.trio.be_.steps.vow.Ee
		,
		planar.IAngle
	{
		public ByPoints(point.Trio val) : base(val)
		{
		}

		[Obsolete("To avoid order confusing, and to be inline with " + nameof(point.Trio)+",use "+nameof(OfPivotInitialStop), true)]
		public ByPoints( Point1 pivot,Point1 inward, Point1 outward) : base(inward, pivot, outward)
		{
		}
		static public ByPoints OfPivotInitialStop(
Point1 pivot,Point1 inward, Point1 outward
		) {
			return new ByPoints(
				new point.Trio(inward,pivot,outward)
			);
		}
		...
		/// (-tau,tau)
		/// </returns>
		public nilnul.geometry.planar.RotationI1 azimuth => nilnul.geometry.planar.grad_.skid._DirectionX._Direction_assumeDif(ee.b, ee.c)

			-
			nilnul.geometry.planar.grad_.skid._DirectionX._Direction_assumeDif(ee.b, ee.a)
